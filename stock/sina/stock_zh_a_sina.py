...
import requests
from akshare.stock.cons import (hk_js_decode, zh_sina_a_stock_amount_url,
                                zh_sina_a_stock_count_url,
                                zh_sina_a_stock_hfq_url,
                                zh_sina_a_stock_hist_url,
                                zh_sina_a_stock_payload,
                                zh_sina_a_stock_qfq_url, zh_sina_a_stock_url)
...
def stock_zh_a_spot() -> pd.DataFrame:
    ...
    for page in tqdm(
        ...
        r = requests.get(zh_sina_a_stock_url, params=zh_sina_stock_payload_copy,headers=headers)
        print(r)
        if page ==5:
            time.sleep(5)
            print(r)
